resource "aws_launch_configuration" "website_launch_configuration" {
  ...
  # Force destroy of dependent resources
  lifecycle {
...
resource "aws_autoscaling_group" "webinstance_scale_group" {
  # Force recreation of group on new deploy.
  name                      = "website-autoscaler-group-${var.environment}-${aws_launch_configuration.website_launch_configuration.name}"
